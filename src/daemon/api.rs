...
impl ApiHandler {
    ...
    fn create_exec(&self, _id: &str, body: &str) -> Result<String> {
        let exec_id = uuid::Uuid::new_v4().to_string();
        Ok(json!({"Id": exec_id}).to_string())
    }

    fn start_exec(&self, _id: &str, _body: &str) -> Result<String> {
        Ok("".to_string())
    }

    fn inspect_exec(&self, id: &str) -> Result<String> {
        Ok(json!({
            "ID": id,
            "Running": false,
            "ExitCode": 0,
    ...
    fn resize_exec(&self, _id: &str, _path: &str) -> Result<String> {
        Ok("".to_string())
    }
