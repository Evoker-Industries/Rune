...
fn build_native(sh: &Shell, release: bool) -> Result<()> {
    ...
    if release {
        cmd!(
            sh,
            "cargo build --release --bin rune --bin rune-tui --bin runefile-lsp"
        )
        .run()?;
    } else {
        cmd!(
            sh,
            "cargo build --bin rune --bin rune-tui --bin runefile-lsp"
        )
        .run()?;
    }
...
fn release(sh: &Shell) -> Result<()> {
    ...
    cmd!(
        sh,
        "cp -r packaging/systemd/* target/release-artifacts/systemd/"
    )
    .run()?;
    ...
    cmd!(
        sh,
        "cp -r packaging/config/* target/release-artifacts/config/"
    )
    .run()?;
